...
void setup() {
	...
	layout = new HSphereLayout().radius(200).detail(4); 
	//here we change the number of points in the sphere using .detail(int)
	//or use .hDetail(int) and .vDetail(int) to change lat/long detail independently  
	b = new HBox();
	pool.add(b);
